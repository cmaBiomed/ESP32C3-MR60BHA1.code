  ...
  */
  ...
  if(WiFi_config()) {
    ...
    if (MQTT_config()) {
      ...
      Output_serial.print(pos_values.distance);
      Output_serial.println("m.");
      Output_serial.print("At time: ");
      Output_serial.print(pos_values.time_stamp);
      Output_serial.println("s");
      
      String position_json = MQTT_publish_distance(pos_values);
      Output_serial.println("Distance published");
      Output_serial.println(position_json);
      if ( 1 == 1 /* MQTT_wait_for_response()) {
        ...
        if (vitals != nullptr) {
          Output_serial.println(MQTT_publish_vitals(vitals));
...
void loop() {
  ...
  Output_serial.println("s");
  */
  /* Output_serial.print("Distance published as: ");
  Output_serial.println(position_json);
  */
}